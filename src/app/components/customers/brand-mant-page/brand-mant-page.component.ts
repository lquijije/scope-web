...
@Component({selector: 'app-brand-mant-page', templateUrl: './brand-mant-page.component.html', styleUrls: ['./brand-mant-page.component.css']})
...
OnDestroy {
    ...
    constructor(public dialog : MatDialog, private cs : CustomerService, private sc : SupermaketsService, private spinnerService : Ng4LoadingSpinnerService) {}
        ...
        if (myForm.valid) {
            ...
            if (!this.editState) {
                ...
                this.cs.updBrand(this.brand).then(() => {
                    ...
                    this.assocSubscription = this.sc.getAssocBrandsFromExclusiveBrand({id: this.brandOld.id, nombre: this.brandOld.nombre}).subscribe(assocs => {
                        assocs.forEach(item => {
    ...
    delete(brand : IBrand) {
        const dialogRef = this.dialog.open(ConfirmDialogComponent, {
            ...
            data: {
                title: 'Confirmación',
                msg: 'Desea eliminar la marca ' + brand.nombre + '?'
            }
        });
