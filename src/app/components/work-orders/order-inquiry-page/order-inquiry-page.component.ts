...
export class OrderInquiryPageComponent implements OnInit, OnDestroy {
    ...
    if ($("#rd-crea").is(":checked")) {
      if ($("#fedesde").val() && $("#fehasta").val()) {
        this.orderList = this.orderList.filter(e => {
          return (
            e.creacion.toDate().getTime() >
              new Date($("#fedesde").val() + " 00:00:000").getTime() &&
            e.creacion.toDate().getTime() <
              new Date($("#fehasta").val() + " 23:59:59").getTime()
          );
        });
      }
      if ($("#fedesde").val() && !$("#fehasta").val()) {
  ...
  delete(order: any) {
    if (
      ...
      order.estado.nombre !== "INICIADA"
    ) {
      this.openAlert(
        "Scope Alert",
        "Sólo se pueden eliminar ordenes CREADAS o INICIADAS, la orden seleccionada está en estado " +
          order.estado.nombre
      );
      return false;
    }
  ...
  geolocation(order: any) {
    ...
    $("#dvGeoIni")
      .html(`<strong class="text-primary">INICIADA</strong> <iframe src="https://maps.google.com/maps?q=${this.orderCurrent.geolocation_iniciada.latitude},${this.orderCurrent.geolocation_iniciada.longitude}&hl=es;z=14&amp;output=embed"
    width="100%" height="350" frameborder="0" style="border:0;" allowfullscreen="" target="_parent">
    </iframe>`);
    $("#dvGeoFin")
      .html(`<strong class="text-primary">FINALIZADA</strong> <iframe src="https://maps.google.com/maps?q=${this.orderCurrent.geolocation_finalizada.latitude},${this.orderCurrent.geolocation_finalizada.longitude}&hl=es;z=14&amp;output=embed"
    width="100%" height="350" frameborder="0" style="border:0;" allowfullscreen="" target="_parent">
    </iframe>`);
  }
  salir() {
      ...
      this.orderList.forEach(o => {
        o.sku.forEach(d => {
          exportJson.push({
            ...
            Sabor: d.sabor,
            "Cant.Inicial": isNaN(parseInt(d.inicial, 10))
              ? ""
              : parseInt(d.inicial, 10),
            "Cant.Final": isNaN(parseInt(d.final, 10))
              ? ""
              : parseInt(d.final, 10),
            Caras: isNaN(parseInt(d.caras, 10)) ? "" : parseInt(d.caras, 10),
            Sugerido: isNaN(parseInt(d.sugerido, 10))
              ? ""
              : parseInt(d.sugerido, 10),
          ...
          if (order.fotos.length === urlFiles.length) {
            ...
            urlFiles.forEach(file => {
              ...
              JSZipUtils.getBinaryContent(file.url, function(err, data) {
                ...
                zipElem++;
                zip.file(ord.local.nombre + "/" + file.name + ".JPG", data, {
                  binary: true
                });
                if (zipElem === urlFiles.length) {
                  zip.generateAsync({ type: "blob" }).then(function(content) {
                    ...
                    saveAs(
                      content,
                      ord.cadena.nombre + "_" + ord.numero + ".zip"
                    );
                  });
                }
              });
  ...
  editOrder(order: any) {
    ...
    $("#cmbMerchantOrder")
      .val(this.orderCurrent.mercaderista.id)
      .trigger("change");
    $("#cmbPriority")
      .val(
        $(
          "#cmbPriority option:contains(" + this.orderCurrent.prioridad + ")"
        ).val()
      )
      .trigger("change");
  ...
  onSubmit(formNew: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.spinnerService.show();
        this.ow
          .updWorkOrder(this.orderCurrent)
          .then(() => {
            this.spinnerService.hide();
  ...
  downloadImages() {
    ...
    if (this.imagesOrderList.length > 0) {
          ...
          if (imgAr.length === urlFiles.length) {
            ...
            urlFiles.forEach(file => {
              ...
              JSZipUtils.getBinaryContent(file.url, function(err, data) {
                ...
                zipElem++;
                zip.file(
                  file.cadena + "/" + file.local + "/" + file.name + ".JPG",
                  data,
                  { binary: true }
                );
                if (zipElem === urlFiles.length) {
      ...
      this.openAlert(
        "Scope Alert",
        "No encontramos ordenes con fotos en esa fecha"
      );
    }
  }
  actualizar() {
  ...
  updateFirestore() {
    ...
    this.spinnerService.show();
    this.orderSubscription = this.ow
      .getWorkOrdersList(desde, hasta, opcion)
      .subscribe(orders => {
        this.spinnerService.hide();
