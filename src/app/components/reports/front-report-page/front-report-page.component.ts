...
@Component({
  selector: 'app-front-report-page',
  templateUrl: './front-report-page.component.html',
  styleUrls: ['./front-report-page.component.css']
})
export class FrontReportPageComponent implements OnInit {
  desde: string = "";
  hasta: string = "";
  realData: any = [];
  preparedData: any = [];
  ...
  processReport() {
    ...
    let cliente = null;
    let nombreArchivo = 'Informe de Caras.xlsx';
                          ...
                          superChain.locales.forEach(superStore => {
                              superStore.visitas.forEach(visit => {
                                  ...
                                  superChain.tags.forEach(col => {
                                      ...
                                      if (skuMatch.length) {
                                          let sum = skuMatch.map(o => o.caras).reduce((a, c) => { return a + c });
                                          finaldata.push(sum);
                                      } else {
                                          finaldata.push('');
                                      }
